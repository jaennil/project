...
func TestCollectorSendsEachEventOnce(t *testing.T) {
	...
	contents := "" +
		...
		`{"timestamp":"2026-07-17T10:00:01.500Z","type":"response_item","payload":{"type":"function_call","name":"exec_command","call_id":"call-1","arguments":"{}"}}` + "\n" +
	...
	if first.sent.Load() != 2 || second.sent.Load() != 0 {
		t.Fatalf("unexpected send counts: first=%d second=%d", first.sent.Load(), second.sent.Load())
	}
	mu.Lock()
	defer mu.Unlock()
	if len(received) != 2 || received[0].EventType != usageEventType || received[1].EventType != metricsEventType {
